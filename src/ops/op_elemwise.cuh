...
// GPU kernel for unary element-wise operation
...
// Launch GPU kernel for unary element-wise operation
template <typename OpFunc, typename T>
...
// GPU kernel for binary element-wise operation with broadcasting
...
// Launch GPU kernel for binary element-wise operation with broadcasting
template <typename OpFunc, typename AT, typename BT, typename OutT>
...
bool op_allclose(const Tensor<T> &at, Tensor<T> &bt) {
    ...
    for (int i = 0; i < at.h; i++) {
        for (int j = 0; j < at.w; j++) {
            ...
            T a = Index(att, i, j);
            T b = Index(btt, i, j);
            if (std::abs(a - b) > std::max(ISCLOSE_RELTOL * std::max(std::abs(a), std::abs(b)), ISCLOSE_ABSTOL)) {
